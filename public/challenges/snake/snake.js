...
function Block() {
  ...
  this.setPos(Math.floor(Math.random() * height), Math.floor(Math.random() * width));
}
...
const updateGame = (pos) => {
  if (pos.top < 0 || pos.top >= height || pos.left < 0 || pos.left >= width || checkSnakeNode(pos, snake.head)) {
