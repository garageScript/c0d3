...
const renderCard = (id) => {
  ...
  container.onclick = (e) => {
    ...
    if (targetClass.contains('center')) {
      if (!confirm(`Are you sure you want to remove ${data[listId].todos[todoId]}?`))
        return;
      actionList.push({ action: 'add', info: {
    ...
    if (targetClass.contains('swap')) {
      ...
      const newId = takeAction('swap', {listId, todoId, newIndex,
        newId: data[newIndex].todos.length });
      actionList.push({action: 'swap', info: {
        listId: newIndex,
        todoId: newId,
        newIndex: listId,
        newId: todoId,
